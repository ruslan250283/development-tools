...
int main(int argc, char **argv)
{
	...
	FILE *bmp_file;
	int i, j, k, q;
	long width = 196;  //设置图片宽度
	long height = 196; //设置图片高度
	long date_size = width*height;
	BIT_MAP_FILE_HEADER bmp_map_file;
	BIT_MAP_INFO_HEADER bmp_map_info;

	fopen_s(&bmp_file, "test.bmp", "wb+");
	//if ((bmp_file = fopen("test.bmp", "wb+")) == NULL)
	if (bmp_file == NULL)
	{
		printf("Error!\n");
		system("pause");
	...
	QRcode *qrcode = QRcode_encodeString("123123", 8, QR_ECLEVEL_H, QR_MODE_8, 1);
	...
	for (i = 0; i < 196; i++)
	{
		for (j = 0; j < 196; j++)
		{
			if (qrcode->data[(i/4) * 49 + j/4] & 0x01)
			{
