...
//qrcode = QRcode_encodeString("http://www.baidu.com", 2, QR_ECLEVEL_L, QR_MODE_STRUCTURE, 0);

codes = QRcode_encodeStringStructured(str, 1, QR_ECLEVEL_L, QR_MODE_8, 0);
...
while(list != NULL) {
	...
	g_ptOriginPixelDatas->iWidth = 21;
	g_ptOriginPixelDatas->iHeight= 21;
	g_ptOriginPixelDatas->iLineBytes= g_ptOriginPixelDatas->iWidth;
	g_ptOriginPixelDatas->aucPixelDatas = list->code->data;
	
	g_ptZoomPixelDatas->iWidth = 21*4;
	g_ptZoomPixelDatas->iHeight= 21*4;
	g_ptZoomPixelDatas->iLineBytes= g_ptZoomPixelDatas->iWidth;
	g_ptZoomPixelDatas->aucPixelDatas = malloc(sizeof(char)*21*21*4*4);
