...
#
#	/* __NH _1206 _29__ */
#	for in current folder deal with filelist
#
use File::Basename;
use File::Spec;
...
$tmppath = $pwd."tmp";	
$cmd_gcc = "gcc";  #$pwd."/tools/MinGW/bin/gcc.exe"); 
$dfile = $pwd."prj_def.txt";
$ufile = $pwd."prj_undef.txt";

# gcc add -D -U
$gcc_d = &get_def_str($dfile);
$gcc_u = &get_def_str($ufile);
...
# for aapt, copy it to somewhere else.
	...
	$dir =~ s/\.\.\/customfiles\/managed_alps\///;
	my $sep = (($dir =~ /^\//) ? "" : "/");
	...
	my $alps_file = $jfile;
	$alps_file =~ s/\.\.\/customfiles\/managed_alps\///;
	&run_cmd("cp -f $i2file $alps_file");
	&run_cmd("chmod 777 $alps_file");
